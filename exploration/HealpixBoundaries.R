PixBoundariesEquatorialBelt<-function(n, k, interval, start_phi, end_phi, start_theta){
  ...
  index<-0
  while (phi < end_phi){
    tmp<- PixBoundariesNextEquatorialBelt(n, k, phi, theta, delta_phi, delta_theta)
    index<-index+1
    phi<-tmp[[1]]
    theta<-tmp[[2]]
    dTheta_dPhi<-tmp[[3]]
...
PixBoundariesNextEquatorialBelt <- function(n, k,prev_phi,prev_theta,delta_phi,delta_theta){
  ...
  t1<- a+b*phi
  t2<- a-b*phi
  if ((a+b*phi)>= -1 && (a+b*phi)<=1){             #With consideration of the complex values
    theta0<- acos(a+b*phi)
    if ((a-b*phi)>= -1 && (a-b*phi)<=1){
      theta1<-acos(a-b*phi) 
    }else{theta1<-Im(acos(a-b*phi))
    }
  }else{
...
############################
PixBoundariesPolarCap<-function(n, k, interval, start_phi, end_phi, flag){
  
  delta_phi<-interval
  phi<- start_phi-delta_phi
  P<-numeric(0)
  while (phi < end_phi){
    tmp<- PixBoundariesNextPolarCap(n, k, phi, delta_phi, flag)
    phi<-tmp[[1]]
    theta<-tmp[[2]]
    dTheta_dPhi<-tmp[[3]]
    a_temp<-cbind(theta, phi)
...
PixBoundariesNextPolarCap<-function(n, k, prev_phi, delta_phi, flag){
