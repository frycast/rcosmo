...
for (k in seq(-3*Nside,Nside-1,1)){
  ...
  S <- PixBoundariesEquatorialBelt(Nside, k, interval, start_phi,
                                   end_phi, start_theta)
  plotPixels(S)
...
## HELPER FUNCTION 1
plotCapPixels <- function(m, S)
{
  for (m in seq(0,3,1)){
    #Northern Hemisphere
    sph <- data.frame(theta = S[, 1], phi = S[, 2]+pi*m/2)
    C<-sph2car(sph)
    plot3d(C[,"x"],C[,"y"],C[,"z"], col = "black", type = "l", add=TRUE)
...
## HELPER FUNCTION 2
plotPixels <- function(S)
{
  sph <- data.frame(theta = S[,1], phi = S[,2])
  C<-sph2car(sph)
  plot3d(C[,"x"],C[,"y"],C[,"z"], col = "black", type = "l",add=TRUE)
}
