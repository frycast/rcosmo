...
#'@return A list containing a matrix named \code{data} whose columns may include
#'the intensity (I), polarisation (Q, U), PMASKa and TMASK. Other list
#'elements correspond to header information and other metadata.
#'@examples
...
CMBReadFITS <- function(filename = "CMB_map_smica1024.fits", sample.size) {
  ...
  #   col <- as.numeric(rep(btype, sample.size))
  #   len <- naxis2*tfields
  #   r <- 1
  #   for (i in 1:len) {
  #       element <- switch(col[i],
  #                         .Internal(readBin(zz, "integer", 1, 1, FALSE, swap)),   # PMASK,TMASK
  #                         .Internal(readBin(zz, "double", 1, 4, TRUE, swap)) )    # I,Q,U
  #       if (i %in% pix)
  #       {
  ...
  #   }
  #
  #   col <- matrix(col, nrow = sample.size, byrow = TRUE)
  # }
  # else
  # { }
  ...
  # if (!missing(sample.size))
  # {
  #   cmbdat <- list(col = col, pix = pix,  hdr = hdr, resoln = resoln, method = method, coordsys = coordsys,
  #                  ordering = ordering, nside = nside, baddata = baddata, header1 = header1,
  #                  header2 = header2)
  # }
  # else {}
  cmbdat <- list(data = col, colnames = TTYPEn, hdr = hdr, resoln = resoln, method = method,
